...
public class UserController {
    ...
    public String registerNewUser(@Valid @ModelAttribute("user") User user, BindingResult result) {
        ...
        if (!isValidPassword(user.getPassword())) {
            result.rejectValue("password", "error.user", "Your password must be at least 8" +
                    " characters long and contain at least one letter, one number, and one special character among @$!%*#?&.");
            return "register";
        }
